...
import { weakTopics } from "../utils/weakTopicDetector";
...
export default function WeakTopicReview({ results, backResults, startAdaptiveExam }) {
  ...
  const weak = weakTopics(domainScores);

  const incorrectQuestions = results.questions
    .map((question, index) => ({
      question,
      index
    }))
    .filter((item) => results.answers[item.index] !== item.question.answer);

  return (
    <div className="app-container">
      <h1>Weak Topic Review</h1>
      ...
      {incorrectQuestions.length === 0 ? (
        ...
        incorrectQuestions.map((item) => (
          <ReviewQuestion
            ...
            userAnswer={results.answers[item.index]}
          />
        ))
      )}
